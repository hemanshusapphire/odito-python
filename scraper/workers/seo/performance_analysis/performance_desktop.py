"""Performance Desktop Analysis Worker."""
...
def execute_performance_desktop_logic(job):
    """Execute PERFORMANCE_DESKTOP job logic"""
    job_id = job.jobId
    project_id = job.projectId
    source_job_id = job.sourceJobId
    
    print(f"[WORKER] PERFORMANCE_DESKTOP started | jobId={job_id} | sourceJobId={source_job_id}")
    ...
    try:
        ...
        if not pages:
            ...
            send_progress_update(job_id, 100, "PERFORMANCE_DESKTOP", "No pages found", "0 pages processed")
            return {
                "status": "completed",
                "jobId": job_id,
        ...
        send_progress_update(job_id, 20, "PERFORMANCE_DESKTOP", f"Found {len(pages)} pages to analyze", f"Processing {len(pages)} pages for desktop performance")
        ...
        for i, page in enumerate(pages):
            ...
            try:
                ...
                performance_data = {
                    ...
                    "first_contentful_paint": 0.8,  # Placeholder
                    "largest_contentful_paint": 1.5,  # Placeholder
                    "cumulative_layout_shift": 0.05,  # Placeholder
                    "total_blocking_time": 80,  # Placeholder
                    "speed_index": 1.2,  # Placeholder
                    "time_to_interactive": 1.8,  # Placeholder
                    "status": "completed",
                    "error": None
                }
                ...
                send_progress_update(job_id, progress, "PERFORMANCE_DESKTOP", f"Analyzed {i + 1}/{len(pages)} pages")
        ...
        print(f"[WORKER] PERFORMANCE_DESKTOP completed | jobId={job_id} | analyzed={len(analyzed_pages)} | failed={len(failed_pages)}")
        ...
        print(f"[ERROR] PERFORMANCE_DESKTOP failed | jobId={job_id} | error=\"{str(e)}\"")
